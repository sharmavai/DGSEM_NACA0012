...
rho = Q(1,1,1);  ru = Q(2,1,1);  rv = Q(3,1,1);  E = Q(4,1,1);
u = ru/rho;  v = rv/rho;
...
for alpha_deg = [0 2 5 10 -5]
    Q  = initialize_solution(mesh, Mach, alpha_deg, gamma, N);
    rho = Q(1,1,1); u=Q(2,1,1)/rho; v=Q(3,1,1)/rho;
    V   = sqrt(u^2+v^2);
...
rho=Q(1,1,1); u=Q(2,1,1)/rho; v=Q(3,1,1)/rho; E=Q(4,1,1);
...
Q = initialize_solution(mesh, Mach, 5.0, gamma, N);
assert(max(abs(diff(Q(1,:,1))))<1e-15,'FAIL: rho not uniform');
assert(max(abs(diff(Q(2,:,1))))<1e-15,'FAIL: rho*u not uniform');
fprintf('TEST 5 (uniform):   PASS\n\n');

fprintf('=== ALL TESTS PASSED ===\n');
fprintf('Safe to proceed to Fix #5 (boundary conditions)\n');
